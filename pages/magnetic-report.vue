...
export default {
  ...
  computed: {
    periods() {
      return this.iframeObject.productsGroupedByMonth.map((month) => moment(month.month).format('MMMYYYY'))
    },
    generatingPercent() {
  ...
  methods: {
    ...
    async generateReport () {
      ...
      const productName = this.iframeObject.productsGroupedByMonth[0]?.products[0]?.mediaReport?.mediaReport?.Network_name__c.replace('.', '_');

      var opt = {
        filename: `Magnetic_${productName}_${moment(this.iframeObject.productsGroupedByMonth[0]?.month).format('YYYYMM')}`,
